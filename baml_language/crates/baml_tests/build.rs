...
";
    ...
    .unwrap_or_else(|error| panic!("write {}: {error}", prof_path.display()));
}
...
fn generate_tests(manifest_dir: &str) {
    ...
    // Prefer the source directory so that file!() returns a stable path
    // (OUT_DIR contains a hash that changes between builds, causing noisy
    // diffs in insta snapshot metadata). Under a per-crate build system
    // (the nix unit graph) the manifest dir is a read-only store path, so
    // fall back to OUT_DIR there; `write_generated_tests` picks whichever
    // works and tells the crate where it landed through a rustc-env, so the
    // `include!` in src/lib.rs follows either way.
...
";
    ...
    write_generated_tests(&dest_path, test_modules, header);
}

/// Write the generated tests, preferring `dest_path` and falling back to
/// `OUT_DIR` when that tree is read-only, then publish the winning path as
/// `BAML_GENERATED_TESTS` for the `include!` in src/lib.rs.
fn write_generated_tests(dest_path: &Path, code: TokenStream, header: &str) {
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let fallback = out_dir.join("generated_tests.rs");
    let written = match write_formatted_code(dest_path, &code, header) {
        Ok(()) => dest_path.to_path_buf(),
        Err(error) => {
            println!(
                "cargo:warning=baml_tests: {} is not writable ({error}), generating into OUT_DIR \
                 instead; insta snapshot paths will carry the OUT_DIR hash",
                dest_path.display()
            );
            write_formatted_code(&fallback, &code, header)
                .unwrap_or_else(|error| panic!("write {}: {error}", fallback.display()));
            fallback
        }
    };
    println!("cargo:rustc-env=BAML_GENERATED_TESTS={}", written.display());
}

fn write_formatted_code(path: &Path, code: &TokenStream, header: &str) -> std::io::Result<()> {
